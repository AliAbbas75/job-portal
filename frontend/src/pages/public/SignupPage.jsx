...
export default function SignupPage() {
  ...
  async function sendOtp(event) {
    ...
    if (!password || password.length < 6) found.password = 'Password must be at least 6 characters';
    ...
    setFormError(null);
    try {
      await requestOtp({ purpose: 'signup', cnic, mobile, operator, password });
      setStep('otp');
    } catch (err) {
      setFormError(err);
    } finally {
  ...
  async function verify(otp) {
    const session = await verifyOtp({ purpose: 'signup', cnic, mobile, otp, password });
    signIn(session);
    ...
    return (
      <AuthCard footer={loginLink}>
        <OtpStep
          mobile={mobile}
          onVerify={verify}
          onResend={() => requestOtp({ purpose: 'signup', cnic, mobile, operator, password })}
  ...
  return (
    <AuthCard
      ...
      lead="New account requires CNIC and verified mobile number only."
      footer={loginLink}
    >
